...
class ShopProvider extends ChangeNotifier {
  static const _coinsKey = 'ld_coins';
  static const _ownedKey = 'ld_owned_items';
  static const _activeThemeKey = 'ld_active_theme';
  static const _activeBgKey = 'ld_active_background';
  static const _activeSkinKey = 'ld_active_skin';
  static const _lastDailyKey = 'ld_last_daily_reward';
  static const _taskRewardDateKey = 'ld_task_reward_date';
  static const _taskRewardCountKey = 'ld_task_reward_count';
  static const _goalRewardDateKey = 'ld_goal_reward_date';
  static const _goalRewardCountKey = 'ld_goal_reward_count';
  static const _streakRewardDateKey = 'ld_streak_reward_date';
  static const _streakRewardCountKey = 'ld_streak_reward_count';
  static const _processedPurchasesKey = 'ld_processed_purchases';
  static const _lastSpinKey = 'ld_last_spin_date';
  static const _loginStreakKey = 'ld_login_streak';
  static const _lastStreakDateKey = 'ld_last_streak_date';
  static const _firstPurchaseBonusKey = 'ld_first_purchase_bonus_used';
  ...
  /// Cheapest shop item not yet owned (excludes remove_ads coin-only path).
  ShopItem? get nextUnlockItem {
    ShopItem? cheapest;
    for (final item in ShopCatalog.items) {
      if (_ownedItems.contains(item.id)) continue;
  ...
  bool get hasExportData => _ownedItems.contains('feat_export');
  bool get hasUnlimitedCountdown => _ownedItems.contains('feat_unlimited_countdown');
  bool get hasPremiumCharts => _ownedItems.contains('feat_premium_charts');
  bool get hasExpensePro => _ownedItems.contains('feat_expense_pro');
  ...
  int countdownLimit(bool hasPremium) =>
      hasPremium ? IapConstants.premiumCountdownLimit : IapConstants.freeCountdownLimit;

  int goalLimit(bool hasPremium) =>
      hasPremium ? IapConstants.premiumGoalLimit : IapConstants.freeGoalLimit;

  bool canAddCountdown(int currentCount) => currentCount < countdownLimit(hasUnlimitedCountdown);
  bool canAddGoal(int currentCount) => currentCount < goalLimit(hasUnlimitedCountdown);
  ...
  Future<bool> claimDailyReward() async {
    ...
    final streakBonus = (_loginStreak.clamp(1, 7) - 1) * 2;
    final amount = IapConstants.dailyLoginReward + streakBonus;
    _coins += amount;
  ...
  /// Returns coins won, or 0 if already spun today.
  ...
  Future<bool> rewardForTaskComplete() => _dailyReward(
        dateKey: _taskRewardDateKey,
        countKey: _taskRewardCountKey,
        max: IapConstants.maxTaskRewardsPerDay,
        amount: IapConstants.taskCompleteReward,
        messageKey: 'taskRewardEarned',
      );

  Future<bool> rewardForGoalMilestone() => _dailyReward(
        dateKey: _goalRewardDateKey,
        countKey: _goalRewardCountKey,
        max: IapConstants.maxGoalRewardsPerDay,
        amount: IapConstants.goalMilestoneReward,
        messageKey: 'goalRewardEarned',
      );
